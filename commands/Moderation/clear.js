...
module.exports = {
	...
	async execute(client, interaction) {
        ...
        if(amount > 100 || amount < 1) {
            await f.sendMessage(`Amount must be in range 0-100!`, embedRed, interaction, "reply", true);
            return;
        }
        if(!interaction.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) {
            await f.sendMessage(`You are missing the Manage Messages permissions!`, embedRed, interaction, "reply", false);
            return;
        }
